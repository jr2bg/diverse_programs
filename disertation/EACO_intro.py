# EVOLUTIONARY ALGORITHM WITH COALITIONS
# NO NEIGHBORHOOD REQUIRED

# VALUACIÓN PARA CADA COALICION
# VALUATION(C_i) = alpha * size(C_i) + beta * var(C_i) + gamma * avg(C_i)
# con
# alpha + beta + gamma = 1
# size(C_i) -> número de elementos/integrantes de C_i
# var(C_i) -> varianza
# avg(C_i) -> promedio
...
class CEA:
    def __init__(self, n_cols, n_rows, alpha, beta, gamma,
                mutation_prob = 0.001, independence_coef = 0.5):
        self.n_rows = n_rows
        self.n_cols = n_cols
        self.grid = np.zeros([n_rows,n_cols])
        self.p_m = mutation_prob
        # propios de EACO
        self.indep_c = independence_coef
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.coalitions = []
        #self.coalitions_vals = []

    def Coalitions_valuation(self):
        ''' función que permite calcular la valuation de cada coalición, donde
        las coaliciones son listas dentro de la lista coalitions'''
        self.coalitions_vals = []

        # para cada lista de coaliciones
        for coalition in self.coalitions:
            avg = sum(coalition) / len(coalition)
            var = sum((xi - avg) ** 2 for xi in coalition) / len(coalition)

            valuation = self.alpha * len(coalition) + self.beta * var +\
                        self.gamma * avg
            self.coalitions_vals.append(valuation)
    ...
    def init_population(self):
        ...
        with np.nditer(self.grid, op_flags = ['readwrite']) as it:
            for cell in it:
                ...
                cell[...] = random_value
                # al inicio, cada célula es su propia coalición
                self.coalitions.append([random_value])
        print(self.grid)
